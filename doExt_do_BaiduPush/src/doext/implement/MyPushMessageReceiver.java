...
public class MyPushMessageReceiver extends PushMessageReceiver {
	...
	public void onBind(Context context, int errorCode, String appid, String userId, String channelId, String requestId) {
		...
		try {
			// errorCode:"绑定接口返回值，0-成功",appId:"",userId:"",channelId:""
			...
			module.getEventCenter().fireEvent("bind", jsonResult);
		} catch (Exception e) {
			DoServiceContainer.getLogEngine().writeError("doBaiduPush=> bind", e);
			e.printStackTrace();
		}
	...
	public void onMessage(Context context, String message, String customContentString) {
		...
		/*
		 * if (!TextUtils.isEmpty(customContentString)) { JSONObject customJson
		 * = null; try { customJson = new JSONObject(customContentString);
		 * String myvalue = null; if (!customJson.isNull("mykey")) { myvalue =
		 * customJson.getString("mykey"); } } catch (JSONException e) { // TODO
		 * Auto-generated catch block e.printStackTrace(); } }
		 */
		// Demo更新界面展示代码，应用请在这里加入自己的处理逻辑
		try {
			// "message":"推送的消息",customContent:"自定义内容，为空或者json字符串"
			String typeId = do_BaiduPush_App.getInstance().getModuleTypeID();
			DoModule module = DoServiceContainer.getSingletonModuleFactory().getSingletonModuleByID(null, typeId);
			DoInvokeResult jsonResult = new DoInvokeResult(module.getUniqueKey());
			JSONObject json = new JSONObject();
			...
			DoServiceContainer.getLogEngine().writeError("doBaiduPush=> message", e);
			e.printStackTrace();
		}
...
//        updateContent(context, responseString);
		if (errorCode == 0) {
			onTagsResult("setTagsResult", true);
		} else {
			onTagsResult("setTagsResult", false);
		}
	}
...
//        updateContent(context, responseString);
		if (errorCode == 0) {
			onTagsResult("removeTagssResult", true);
		} else {
			onTagsResult("removeTagssResult", false);
		}
	}

	private void onTagsResult(String _messageName, boolean _result) {
		try {
			String typeId = do_BaiduPush_App.getInstance().getModuleTypeID();
			DoModule module = DoServiceContainer.getSingletonModuleFactory().getSingletonModuleByID(null, typeId);
			DoInvokeResult invokeResult = new DoInvokeResult(module.getUniqueKey());
			invokeResult.setResultBoolean(_result);
			module.getEventCenter().fireEvent(_messageName, invokeResult);
		} catch (Exception e) {
			DoServiceContainer.getLogEngine().writeError(_messageName, e);
			e.printStackTrace();
		}
	}
...
//        updateContent(context, responseString);
	...
	public void onUnbind(Context context, int errorCode, String requestId) {
		...
		try {
			...
			json.put("errorCode", errorCode);
			jsonResult.setResultNode(json);
			module.getEventCenter().fireEvent("unbind", jsonResult);
		} catch (Exception e) {
			DoServiceContainer.getLogEngine().writeError("doBaiduPush=> unbind", e);
			e.printStackTrace();
		}
		// updateContent(context, responseString);
	}
...
//        intent.setClassName(context.getApplicationContext(), "com.baidu.push.example.LoginActivity");
//        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
